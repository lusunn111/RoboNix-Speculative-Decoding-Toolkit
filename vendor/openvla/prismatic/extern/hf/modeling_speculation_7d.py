...
from .utils import *
from experiments.robot.tool_utils import kalman_predict_from_history, kalman_predict_7d_from_history
...
class SpecVLAforActionPrediction(nn.Module):
    ...
    def __init__(
        ...
        self.cnt_sig = 0 
        self.cnt_total = 0 # 总步数统计（每次eagenerate一次动作计1步）
        self._episode_started = False  # 本episode是否已经有生成（用于上一次episode汇总打印）
    ...
    def start_rollout(
        ...
        *,
        ...
        self._step_idx = -1
        self._token_history = []
        # Print summary for the previous episode (task) if it has started
        try:
            if self._episode_started:
                print(f"SpecDecoding task summary: steps={self.cnt_total}, kf_steps={self.cnt_kf}")
        except Exception:
            pass
    ...
    def save_error_stats(self) -> None:
        ...
        # Print KF usage summary
        try:
            print(f"SpecDecoding summary: total_steps={self.cnt_total}, kf_steps={self.cnt_kf}")
        except Exception:
            pass
    ...
    def kelman_predict_tokens_7D(
        self,
        *,
        obs_tokens: Sequence[int],
    ) -> List[int]:
        """Run a 7D Kalman filter using full-vector history + current observation.

        - History: previously accepted action token vectors (one per step).
        - Observation: length-``action_dim`` vector composed of verified tokens for
          the accepted prefix and the best-candidate tail for the remainder.

        Returns a full-length token vector (ints) for the current step.
        """
        ...
        # Clamp/normalize observation to the right length
        obs_list = [int(t) for t in list(obs_tokens or [])]
        if len(obs_list) < action_dim:
            # Pad with last token or pad_token if missing
            pad_token = getattr(self.tokenizer, "pad_token_id", None)
            action_token_start = self.vocab_size - self.bin_centers.shape[0]
            token_min = action_token_start
            token_max = self.vocab_size - 1
            if pad_token is None:
                pad_token = token_min
            pad_token = int(max(token_min, min(token_max, pad_token)))
            fallback = obs_list[-1] if obs_list else pad_token
            obs_list = obs_list + [int(fallback)] * (action_dim - len(obs_list))
        elif len(obs_list) > action_dim:
            obs_list = obs_list[:action_dim]

        # Build history vectors
        history_vectors: List[List[float]] = []
        # Prefer the lightweight ring-buffer built during eager generation
        if getattr(self, "_token_history", None):
            history_vectors = [
                [float(int(x)) for x in row[:action_dim]]
                for row in list(self._token_history)
                if isinstance(row, (list, tuple)) and len(row) >= action_dim
            ]
        else:
            # Fallback to accepted_actions-based history if available
            try:
                acts = self.get_history()
                for seq in acts:
                    if isinstance(seq, (list, tuple)) and len(seq) >= action_dim:
                        history_vectors.append([float(int(x)) for x in seq[:action_dim]])
            except Exception:
                history_vectors = []

        # Apply history windowing
        window = getattr(self, "_kalman_history_window", None)
        if window is not None and window > 0 and len(history_vectors) > window:
            history_vectors = history_vectors[-int(window):]

        # Kalman parameters
        process_var = self._kalman_config.get("process_var")
        measurement_var = self._kalman_config.get("measurement_var")
        default_process = float(process_var) if process_var is not None else 1.0
        default_measure = float(measurement_var) if measurement_var is not None else 1e-3

        # Run 7D (N-D) diagonal KF filtering with current observation
        try:
            filtered_vec = kalman_predict_7d_from_history(
                history_vectors,
                observation=[float(int(t)) for t in obs_list],
                process_variance=default_process,
                measurement_variance=default_measure,
            )
        except Exception:
            # On failure, just return obs_list
            filtered_vec = [float(int(t)) for t in obs_list]

        # Convert to valid token ids
        action_token_start = self.vocab_size - self.bin_centers.shape[0]
        token_min = action_token_start
        token_max = self.vocab_size - 1
        out_tokens = []
        for v in filtered_vec[:action_dim]:
            tok = int(round(v))
            tok = max(token_min, min(token_max, tok))
    ...
    def _finalize_action_with_kf(
        ...
        fallback_token = clamp_token(
            accepted_tokens[-1] if accepted_tokens else (candidate_tail[0] if candidate_tail else pad_token)
        )

        # Build observation: verified prefix + best-candidate tail (pad as needed)
        total_dims = int(action_dim)
        obs_vec: List[int] = []
        obs_vec.extend(accepted_tokens)
        needed_tail = max(total_dims - len(obs_vec), 0)
        if needed_tail > 0:
            tail_fill = candidate_tail[:needed_tail]
            if len(tail_fill) < needed_tail:
                tail_fill.extend([fallback_token] * (needed_tail - len(tail_fill)))
            obs_vec.extend([clamp_token(int(t)) for t in tail_fill])
        if len(obs_vec) > total_dims:
            obs_vec = obs_vec[:total_dims]

        # Run 7D KF to produce a full vector for this step
        try:
            kf_full_vec = self.kelman_predict_tokens_7D(obs_tokens=obs_vec)
        except Exception:
            kf_full_vec = obs_vec
        if not kf_full_vec:
            kf_full_vec = obs_vec
        kf_full_vec = [clamp_token(int(t)) for t in kf_full_vec[:total_dims]]

        # Compose final tokens: keep verified prefix, fill rest from KF vector
        combined_tokens = accepted_tokens[:]
        if len(combined_tokens) < action_dim:
            remainder = kf_full_vec[len(combined_tokens):]
            combined_tokens.extend(remainder)
    ...
    def eagenerate(
        ...
        **kwargs
        ...
        # update counters after one action finished
        try:
            self.cnt_total += 1
            if used_kf:
                self.cnt_kf += 1
