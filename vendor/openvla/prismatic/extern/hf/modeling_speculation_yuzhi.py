...
class SpecVLAforActionPrediction(nn.Module):
    ...
    def _finalize_action_with_kf(
        ...
        accept_length: int,
        kf_predictions: Optional[Sequence[int]] = None,
    ...
    def eagenerate(
        ...
        **kwargs
        ...
        for idx in range(max_length):
            ...
            use_kf = False
            kf_predictions: List[int] = []
            if self._kf_consecutive < 3:
                try:
                    kf_predictions = self.kelman_predict_tokens()
                except Exception:
                    kf_predictions = []
            if kf_predictions:
                try:
                    action_dim_eval = int(self.get_action_dim())
                except Exception:
                    action_dim_eval = 0
                if action_dim_eval > 0 and len(kf_predictions) >= action_dim_eval:
                    token_min = self.vocab_size - self.bin_centers.shape[0]
                    zero_offset = (32000 - 31744) / 2.0
                    offsets = [
                        ((int(tok) - token_min) - zero_offset)
                        for tok in kf_predictions[:action_dim_eval]
                    ]
                    mean_offset = sum(offsets) / len(offsets)
                    if abs(mean_offset) <= 30 and self._kf_consecutive < 3:
                        use_kf = True
            if use_kf:
